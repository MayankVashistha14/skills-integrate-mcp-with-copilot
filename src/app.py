...
with teacher_file.open(encoding="utf-8") as file:
...
def login(request: LoginRequest):
    teacher = next(
        (
            ...
            if hmac.compare_digest(teacher["username"], request.username)
            and hmac.compare_digest(teacher["password"], request.password)
        ),
        None,
    )
    if teacher is None:
